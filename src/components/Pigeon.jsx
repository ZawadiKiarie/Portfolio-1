...
export function Pigeon(props) {
  ...
  useEffect(() => {
    // actions can be undefined early; the specific action may be missing momentarily
    const action = actions?.[animation];
    if (!action) return; // nothing to start/cleanup yet

    action.reset().fadeIn(0.5).play();

    return () => {
      // on cleanup, fade out exactly the action we started
      action.fadeOut(0.5);
    };
  }, [actions, animation]);
